...
const Transactions: React.FC = () => {
  ...
  async function handleSaveTransaction(transactionData: any) {
    try {
      ...
      if (transactionData.id) {
        ...
        await updateAccountBalances(cleanTransactionData.transaction_type, amountMinor, cleanTransactionData.from_account_id, cleanTransactionData.to_account_id);
      }

      console.log('Transaction saved successfully!');
      
      // Refresh both accounts and transactions to reflect changes
      await loadAccounts();
  ...
  async function handleDeleteTransaction(transaction: Transaction) {
    ...
    try {
      // First update account balances (reverse the transaction)
  ...
  // Function to reverse account balances when deleting a transaction
  async function updateAccountBalancesForDelete(transaction: Transaction) {
    try {
  ...
  // Function to update account balances based on transaction
  async function updateAccountBalances(transactionType: string, amountMinor: number, fromAccountId: string | null, toAccountId: string | null) {
    try {
      if (transactionType === 'income' && toAccountId) {
        // Add amount to the 'to' account for income
        const { data: currentAccount, error: fetchError } = await supabase
          .from('accounts')
          .select('balance_minor')
          .eq('id', toAccountId)
        ...
        const { error } = await supabase
          .from('accounts')
          .update({
            balance_minor: (currentAccount?.balance_minor || 0) + amountMinor
          })
          .eq('id', toAccountId);

        if (error) throw error;
      } 
      else if (transactionType === 'expense' && fromAccountId) {
        // Subtract amount from the 'from' account for expense
        const { data: currentAccount, error: fetchError } = await supabase
          .from('accounts')
          .select('balance_minor')
          .eq('id', fromAccountId)
        ...
        // Get current balances for both accounts
        const [fromAccountResult, toAccountResult] = await Promise.all([
          supabase.from('accounts').select('balance_minor').eq('id', fromAccountId).single(),
          supabase.from('accounts').select('balance_minor').eq('id', toAccountId).single()
        ]);
        ...
        const { error: fromError } = await supabase
          .from('accounts')
          .update({
            balance_minor: (fromAccountResult.data?.balance_minor || 0) - amountMinor
          })
          .eq('id', fromAccountId);
        ...
        const { error: toError } = await supabase
          .from('accounts')
          .update({
            balance_minor: (toAccountResult.data?.balance_minor || 0) + amountMinor
          })
          .eq('id', toAccountId);
